...
class BKServer {
  ...
  constructor(port: number, mainWindow: BrowserWindow) {
    ...
    this.handoffService = new HandoffService(mainWindow, this.loggerService);
  ...
  private setupRoutes(): void {
    this.app.get('/api/v1/handoff/list', (_req, res) => {
      res.json({ success: true, data: this.handoffService.list() });
    });

    this.app.post('/api/v1/handoff/ack', (req, res) => {
      const { id } = req.body || {};
      const ok = typeof id === 'string' && this.handoffService.ack(id);
      res.json({ success: ok });
    });

    this.app.post('/api/v1/handoff/resume', (req, res) => {
      const { sessionKey } = req.body || {};
      const ok =
        typeof sessionKey === 'string' &&
        this.handoffService.resume(sessionKey);
      res.json({ success: ok });
    });

    this.app.post('/api/v1/handoff/clear-cooldowns', (_req, res) => {
      const n = this.handoffService.clearAllCooldowns();
      res.json({ success: true, data: { cleared: n } });
    });
    ...
    // 绑定网关 Shop
    this.app.post('/api/v1/strategy/task/bind-shop', async (req, res) => {
      const { taskId, gatewayShopId } = req.body;
      try {
        const ok = await this.appService.bindGatewayShop(
          String(taskId),
          gatewayShopId ? String(gatewayShopId) : null,
        );
        res.json({ success: ok });
      } catch (error) {
        res.json({
    ...
    // 按扫码店名自动创建／绑定网关 Shop
    this.app.post('/api/v1/strategy/task/ensure-shop', async (req, res) => {
      const { taskId } = req.body;
      try {
        const { ensureInstanceGatewayShop } = await import(
          './services/ensureInstanceGatewayShop'
        );
        const r = await ensureInstanceGatewayShop(String(taskId));

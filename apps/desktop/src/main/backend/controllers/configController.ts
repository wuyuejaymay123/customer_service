  ...
  > {
    ...
    if (type === 'generic') {
      return {
        ...
        waitHumansTime: config?.wait_humans_time || 60,
        ...
        failureHandoffReply:
          config?.failure_handoff_reply || '稍等，我帮您找同事看一下',
        handoffCooldownSeconds: config?.handoff_cooldown_seconds || 900,
      };
    }
  ...
  public async updateConfigByType({
    ...
    if (type === 'generic') {
      ...
      await dbConfig.update({
        ...
        failure_handoff_reply: config.failureHandoffReply,
        handoff_cooldown_seconds: config.handoffCooldownSeconds,
      });
    } else if (type === 'llm') {

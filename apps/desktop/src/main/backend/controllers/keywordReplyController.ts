...
可使用的平台名称：${
  ...
  async getKeywords(platformId: string, shopId?: string | null) {
    ...
    const platformKeywords = await Keyword.findAll({
      where: {
        platform_id: platformId,
      },
    });

    const all = [...globalKeywords, ...platformKeywords];
    if (!shopId) {
      // 未绑 Shop：只用 shop_id 为空的（渠道／全局），避免串他店素材
      return all.filter((k) => !k.shop_id);
    }
    return all.filter((k) => !k.shop_id || k.shop_id === shopId);
  }

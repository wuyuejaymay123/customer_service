...
export class Config extends Model {
  ...
  /** 失败转人工安抚语（不露馅；独立于 default_reply） */
  declare failure_handoff_reply: string;

  /** 会话冷却秒数（FailureHandoff 后该买家暂停自动回） */
  declare handoff_cooldown_seconds: number;
}
...
export async function checkAndAddFields(sequelize: Sequelize) {
  ...
  // @ts-ignore
  if (!tableDescription.failure_handoff_reply) {
    await sequelize
      .getQueryInterface()
      .addColumn('n_config', 'failure_handoff_reply', {
        type: DataTypes.TEXT,
        allowNull: true,
        defaultValue: '稍等，我帮您找同事看一下',
      });
  }

  // @ts-ignore
  if (!tableDescription.handoff_cooldown_seconds) {
    await sequelize
      .getQueryInterface()
      .addColumn('n_config', 'handoff_cooldown_seconds', {
        type: DataTypes.INTEGER,
        allowNull: true,
        defaultValue: 900,
      });
  ...
  // 清除「消息有点多／稍后再回复」类话术，禁止再发给买家
  await sequelize.query(
    `UPDATE n_config SET default_reply = '稍等，我帮您确认一下'
     WHERE default_reply IS NOT NULL AND (
       default_reply LIKE '%消息有点多%'
       OR default_reply LIKE '%稍后再回复%'
       OR default_reply LIKE '%稍后再回%'
     )`,
  );
}

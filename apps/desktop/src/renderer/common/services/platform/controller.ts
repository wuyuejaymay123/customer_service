...
export async function ackHandoff(id: string) {
  await POST('/api/v1/handoff/ack', { id });
}

export async function resumeHandoffSession(sessionKey: string) {
  await POST('/api/v1/handoff/resume', { sessionKey });
}
...
export async function bindInstanceShop(
  taskId: string,
  gatewayShopId: string | null,
) {
  const data = await POST<{ success?: boolean; error?: string }>(
    `/api/v1/strategy/task/bind-shop`,
    { taskId, gatewayShopId },
  );
  if (data && typeof data === 'object' && data.success === false) {
    throw new Error(data.error || '绑定店铺失败');
  }
  return data;
}

export async function ensureInstanceShop(taskId: string) {
  const data = await POST<{
    success?: boolean;
    error?: string;
    data?: {
  ...
  }>(`/api/v1/strategy/task/ensure-shop`, { taskId });
  if (data && typeof data === 'object' && data.success === false) {
    throw new Error(data.error || '自动绑定店铺失败');
  }
  return data;
}
